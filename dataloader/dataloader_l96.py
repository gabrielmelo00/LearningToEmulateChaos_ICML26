...
class TrainingData(Dataset):
    def __init__(self, crop_T, convert_to_pil = False, train_size = 1000, \
        ...
        if self.convert_to_pil:
            ...
            self.data_list.sort()
            self.params_array_list = torch.load('{}/training_params.pth'.format(self.data_path), weights_only=False)
        else:
            ...
            self.data_list.sort()
            self.params_array_list = torch.load('{}/training_params.pth'.format(data_path), weights_only=False)
            self.nan_list = torch.load('{}/nan_list.pth'.format(data_path), weights_only=False)
    ...
    def __getitem__(self, idx):
        ...
        if self.convert_to_pil:
            anchor_d = torch.load(self.data_list[idx], weights_only=False)
