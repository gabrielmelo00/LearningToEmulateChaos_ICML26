...
class SpectralConv1d(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        n_modes = min(self.modes1, x_ft.size(-1), self.weights1.size(2))
        weights = torch.view_as_complex(self.weights1[:, :, :n_modes, :])
        out_ft[:, :, :n_modes] = self.compl_mul1d(x_ft[:, :, :n_modes], weights)
...
class FNO1d(nn.Module):
    def __init__(self, modes, width):
        ...
        proj_dim = max(256, self.width * 4)
        self.fc1 = nn.Linear(self.width, proj_dim)
    ...
    def forward(self, x, param=None):
        ...
        if param is None:
            param = torch.zeros((b, 1), device=x.device, dtype=x.dtype)
        else:
            if param.dim() == 1:
                param = param[:, None]
            else:
                param = param.reshape(b, -1)
            if param.shape[1] != 1:
                raise ValueError(f'FNO1d expects scalar conditioning, got shape {tuple(param.shape)}')
