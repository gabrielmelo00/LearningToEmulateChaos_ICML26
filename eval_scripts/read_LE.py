...
baseline = torch.load(baseline, weights_only=False)
cl = torch.load(cl, weights_only=False)
ot = torch.load(ot, weights_only=False)
truth = np.array(torch.load(truth, weights_only=False))
